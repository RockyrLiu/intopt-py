...
class BinaryPSO:
    """离散粒子群优化算法(0-1问题)"""
    def __init__(self, func, popsize=30, maxiter=100, 
                 w=0.8, w_min=0.4, c1=1.5, c2=1.5, v_max=4,
                 init_positions=None, verbose=True):
        """
        参数说明:
        func: 目标函数(适应度函数)
        ...
        # 通过测试函数自动确定维度
        test_input = np.ones((1, 1))  # 初始测试输入
        try:
            test_output = func(test_input)
            self.dim = 1
        except:
            # 如果维度为1不适用，尝试更通用的方法
            test_input = np.ones((1, 10))  # 使用更大的测试维度
            test_output = func(test_input)
            self.dim = test_input.shape[1]
        
        # 初始化粒子位置和速度
        if init_positions is not None:
            assert init_positions.shape[0] == popsize, "初始位置样本数错误"
            self.dim = init_positions.shape[1]  # 从初始位置获取维度
            self.X = init_positions.copy()
        else:
            self.X = np.random.randint(0, 2, (popsize, self.dim))  # 二进制位置(0或1)
        ...
        self.pbest = self.X.copy()
        self.p_fit = self.function(self.X)
        max_idx = np.argmax(self.p_fit)
        self.fit = self.p_fit[max_idx]
        self.gbest = self.X[max_idx].reshape(1, -1).copy()
    ...
    def iterator(self):
        """执行优化迭代"""
        if self.verbose:
            pbar = tqdm(total=self.maxiter, desc="Binary PSO优化进度")
        
        for t in range(self.maxiter):
            # 动态衰减惯性权重
            w = self.w_min + (self.w - self.w_min) * (self.maxiter - t) / self.maxiter
            ...
            # 更新个体和全局最优
            update_mask = current_fit > self.p_fit  # 注意这里是最大化问题
            self.pbest[update_mask] = self.X[update_mask]
            self.p_fit[update_mask] = current_fit[update_mask]
            ...
            if current_fit[max_idx] > self.fit:
                ...
                self.gbest = self.X[max_idx].reshape(1, -1)
...
def binary_pso(func, popsize=30, maxiter=100, 
              w=0.8, w_min=0.4, c1=1.5, c2=1.5, v_max=4,
              init_positions=None, verbose=True, plot=True):
    """
    离散粒子群优化(0-1问题)
    ...
    if plot:
        ...
        plt.xlabel('迭代次数', size=12)
        plt.ylabel('适应度值', size=12)
        plt.title('Binary PSO优化过程 - 适应度变化', fontsize=14)
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.show()
...
def main4():
    ...
    # 调用离散PSO优化
    best_solution, history = binary_pso(
        func=knapsack.fitness,
        popsize=100,
        maxiter=200,
        verbose=True
    )
    ...
    # 评估最优解
    solution_info = knapsack.evaluate_solution(best_solution)
    
    # 输出结果
    print("\n优化结果:")
    print(f"选中的物品索引: {solution_info['selected_indices']}")
    print(f"总价值: {solution_info['total_value']:.2f}")
    print(f"总体积: {solution_info['total_weight']:.2f} (容量: {capacity})")
    print(f"是否可行解: {'是' if solution_info['is_feasible'] else '否'}")
