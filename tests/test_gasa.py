...
def test_gasa_continuous_returns_optimize_result():
    ...
    ga = _ContGASA(problem, pop_size=20, generations=5, verbose=False)
    result = ga.run()
...
def test_gasa_rastrigin_10d_converges():
    from intopt.algorithms.gasa import GASA
    from intopt.operators.crossover import cxArithmetic
    from intopt.operators.initialize import initRandom
    from intopt.operators.mutate import mutGaussian
    ...
    ga = _ContGASA(problem, pop_size=100, generations=200, verbose=False)
    result = ga.run()

    assert result.best_fitness <= 9.0


def test_gasa_tsp_path_is_valid_permutation():
    from intopt.algorithms.gasa import GASA
    ...
    ga = _TspGASA(problem, pop_size=20, generations=30, verbose=False)
    result = ga.run()
...
def test_gasa_tsp_data_improves():
    from intopt.algorithms.gasa import GASA
    from intopt.operators.crossover import cxOrdered
    from intopt.operators.initialize import initRandom
    from intopt.operators.mutate import mutSwap
    ...
    ga = _TspGASA(problem, pop_size=60, generations=50, verbose=False)
    result = ga.run()

    assert result.best_fitness <= result.history["best"][0]


def test_gasa_early_stopping_terminates_early():
    from intopt.algorithms.gasa import GASA
    ...
    ga = _ContGASA(problem, pop_size=30, generations=200, verbose=False)
